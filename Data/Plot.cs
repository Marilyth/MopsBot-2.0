...
namespace MopsBot.Data
{
    ...
    public class ColumnPlot
    {
        ...
        public string DrawPlot()
        {
            ...
            return $"{Program.Config["ServerAddress"]}/api/Content?name={ID.Replace(" ", "%20")}barplot.png&rand={StaticBase.ran.Next(0, 999999999)}";
        }
        ...
        public string DrawPlot(bool returnPdf = false, string fileName = null, bool path = false)
        {
            ...
            if ((DateTime.UtcNow - lastPlot).TotalMinutes > 1 || returnPdf || path)
            {
                ...
                if (!returnPdf)
                {
                    ...
                    if (!path)
                        return $"{Program.Config["ServerAddress"]}/api/Content?name={fileName}.png&rand={StaticBase.ran.Next(0, 999999999)}";
